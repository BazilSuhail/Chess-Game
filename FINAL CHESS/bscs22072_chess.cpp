...
void chess::draw_boxes(RenderWindow& w) {
	...
	if (!S1.loadFromFile("grey.png") || !S2.loadFromFile("lgrey.png") || !back.loadFromFile("back.png") || !S3.loadFromFile("undo.png") || !S4.loadFromFile("redo.png")) {
...
void chess::select_undo(RenderWindow& window, Texture& S1, loc S) {
	if (S.ri == 7 && S.ci == 14) {
		...
		draw_board_pieces(window, S1);		window.display();
		S = select_src(window);
		if (S.ri == 7 && S.ci == 15) {
			Box->load_game();
...
void chess::actual_highlight(sf::RenderWindow& w, bool** Box, int Turn, sf::Texture& A) {
	Texture t1;
	if (!t1.loadFromFile("highlight.png")) {
		cout << "Failed to load texture!" << endl;
	}
	Font font;
	if (!font.loadFromFile("phantomstorm.ttf")) {
...
void chess::DisplayCheck(RenderWindow& w){
	sf::Font fnt;
	if (!fnt.loadFromFile("phantomstorm.ttf")) {
		cout << "Failed to load font" << endl;
	}
	Text t("", fnt, 30);
...
void DisplayCheck1(RenderWindow& w){
	sf::Font font;
	if (!font.loadFromFile("BAD GRUNGE.ttf")) {
		cout << "Failed to load font" << endl;
	}
	sf::Text t1("     ", font, 50);
